/**
 * Construction V1 Chat Bubble
 * Floating chat interface for AI assistant
 */

class ConstructionChatBubble {
    constructor() {
        this.isOpen = false;
    ...
    async checkConfiguration() {
        try {
            console.log("🔍 Chat bubble: Checking configuration...");
            const response = await frappe.call({
                method: 'construction_v1.services.settings_service.validate_access',
                type: 'GET'
            });
            ...
            if (response.message) {
                ...
                console.log("🔍 has_access:", response.message.has_access);
                console.log("🔍 enabled:", response.message.enabled);

                this.isConfigured = response.message.has_access && response.message.enabled;
                console.log("🔍 isConfigured result:", this.isConfigured);

                if (!response.message.enabled) {
                    this.configError = 'Construction V1 is disabled in settings';
                    console.log("❌ Configuration error: disabled");
                } else if (!response.message.has_access) {
                    this.configError = 'You do not have permission to use Construction V1';
                    console.log("❌ Configuration error: no access");
                } else {
                    console.log("✅ Configuration check passed");
                    // Additional check for API key configuration
                    const settingsResponse = await frappe.call({
                        method: 'construction_v1.services.settings_service.get_settings',
                        type: 'GET'
                    });
                ...
                <div class="exn-chat-window" id="exnChatWindow">
                    ...
                    <div class="exn-chat-header">
                        <div>
                            ...
                            <p class="exn-chat-subtitle">Your Construction AI assistant</p>
                        </div>
                    ...
                    <div class="exn-chat-messages" id="exnChatMessages">
                        <div class="exn-welcome-message">
                            ...
                            <h4>Hi! I'm WorkCom, your Construction assistant!</h4>
                            <p>I'm here to help you with all your Construction needs. Ask me about:</p>
                            <ul style="text-align: left; margin: 10px 0;">
                                ...
                                <li>Any other Construction services</li>
                            </ul>
                        </div>
                    </div>
        ...
        if (window.frappe && frappe.realtime) {
            ...
            this.realtimeEvents.joinRoom = () => {
                if (this.conversationId) {
                    frappe.call({
                        method: 'construction_v1.services.realtime_service.join_conversation_room',
    ...
    shouldUseOptimizedAPI() {
        ...
        const userPreference = localStorage.getItem('construction_v1_api_preference');
    ...
    async callOptimizedChatAPI(message) {
        ...
        try {
            ...
            const response = await frappe.call({
                method: 'construction_v1.api.simplified_chat.send_message',
                args: {
                    message: message,
    ...
    logAPIUsage(apiType, status, responseTime, metadata = {}) {
        ...
        try {
            const logs = JSON.parse(localStorage.getItem('construction_v1_api_logs') || '[]');
            logs.push(logData);
            ...
            localStorage.setItem('construction_v1_api_logs', JSON.stringify(logs));
        } catch (e) {
    ...
    async sendAPILogToServer(logData) {
        ...
        try {
            ...
            setTimeout(async () => {
                try {
                    await frappe.call({
                        method: 'construction_v1.api.simplified_chat.get_chat_status',
    ...
    clearAPIUsageLogs() {
        ...
        try {
            localStorage.removeItem('construction_v1_api_logs');
    ...
    setAPIPreference(preference) {
        ...
        try {
            if (preference === null || preference === 'auto') {
                localStorage.removeItem('construction_v1_api_preference');
                // Verbose preference clearing notification removed
            } else if (preference === 'optimized' || preference === 'legacy') {
                localStorage.setItem('construction_v1_api_preference', preference);
    ...
    getAPIPreference() {
        ...
        try {
            return localStorage.getItem('construction_v1_api_preference') || 'auto';
        } catch (e) {
    ...
    handleAgentAssignment(data) {
        ...
        if (chatTitle) {
            chatTitle.textContent = `${data.agent_name} - Construction Assistant`;
        }
    ...
    sendTypingStatus(isTyping) {
        if (this.conversationId && window.frappe) {
            frappe.call({
                method: 'construction_v1.services.realtime_service.update_typing_status',
                args: {
...
$(document).ready(function() {
    console.log("🔍 Chat bubble DOM ready - checking initialization conditions");
    ...
    if (window.frappe && frappe.boot) {
        console.log("✅ Creating ConstructionChatBubble instance");
        ...
        console.log("✅ ConstructionChatBubble created:", typeof window.ConstructionChatBubble);
    } else {
        console.log("❌ Cannot create chat bubble - frappe not ready");
    }
});

// Also provide a manual initialization function
window.initializeConstructionChatBubble = function() {
    console.log("🔧 Manual chat bubble initialization called");
    ...
    return window.ConstructionChatBubble;
};
